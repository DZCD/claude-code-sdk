...
export class TaskListTool extends BaseTool<typeof taskListSchema, TaskListOutput> {
  ...
  async execute(_input: z.infer<typeof taskListSchema>, _context: ToolContext): Promise<ToolResult<TaskListOutput>> {
    ...
    const resolvedTaskIds = new Set(visibleTasks.filter((t) => t.status === 'completed').map((t) => t.id))
