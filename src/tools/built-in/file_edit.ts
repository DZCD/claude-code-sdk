...
export class FileEditTool extends BaseTool<typeof fileEditSchema, FileEditOutput> {
  name = 'edit'
  description = 'Edit a file by finding and replacing exact text matches. Can also append content by providing an empty old_string.'
  inputSchema = fileEditSchema

  async execute(
    input: z.infer<typeof fileEditSchema>,
    ...
    try {
      ...
      if (nodeErr.code === 'ENOENT') {
        return {
          data: { type: 'create', filePath: file_path, oldString: old_string, newString: new_string },
          content: `Error: File does not exist: ${file_path}`,
          isError: true,
        }
      }
      return {
        data: { type: 'update', filePath: file_path, oldString: old_string, newString: new_string },
    ...
    const newContent =
      ...
      new_string +
      existingContent.slice(index + old_string.length)
