...
describe('BashTool — Timeout', () => {
  ...
  it('should timeout with short timeout for long running command', async () => {
    const result = await tool.execute({ command: 'sleep 5', timeout: 500 }, makeContext())
  ...
  it('should handle extremely short timeout (1ms)', async () => {
    const result = await tool.execute({ command: 'echo hello', timeout: 1 }, makeContext())
  ...
  it('should complete normally with adequate timeout', async () => {
    const result = await tool.execute({ command: 'echo fast', timeout: 5000 }, makeContext())
...
describe('BashTool — Large Output', () => {
  ...
  it('should handle large stdout output', async () => {
    ...
    const result = await tool.execute({ command: 'yes "test line" | head -n 5000' }, makeContext())
    expect(result.isError).toBeFalsy()
  ...
  it('should handle output with special characters', async () => {
    const result = await tool.execute({ command: 'echo "special chars: ~@#%^*()_+-=[]{}|:.<>?/"' }, makeContext())
    expect(result.isError).toBeFalsy()
...
describe('BashTool — Compound Commands', () => {
  ...
  it('should execute chained commands with &&', async () => {
    const result = await tool.execute({ command: 'echo first && echo second && echo third' }, makeContext())
    expect(result.isError).toBeFalsy()
  ...
  it('should capture both stdout and stderr from compound command', async () => {
    const result = await tool.execute({ command: 'echo "stdout msg" && echo "stderr msg" >&2' }, makeContext())
    expect(result.isError).toBeFalsy()
  ...
  it('should return error on first failing command with &&', async () => {
    const result = await tool.execute({ command: 'echo first && exit 1 && echo should_not_run' }, makeContext())
    expect(result.isError).toBe(true)
  ...
  it('should handle pipe between commands', async () => {
    const result = await tool.execute({ command: 'echo "hello world" | wc -w' }, makeContext())
    expect(result.isError).toBeFalsy()
...
describe('BashTool — Semicolon Security', () => {
  ...
  it('should reject commands with semicolons outside quotes', async () => {
    const result = await tool.execute({ command: 'echo a; echo b; echo c' }, makeContext())
    expect(result.isError).toBe(true)
  ...
  it('should reject semicolon with two commands', async () => {
    ...
    const result = await tool.execute({ command: 'echo a; echo b' }, makeContext())
    expect(result.isError).toBe(true)
    expect(result.content).toContain('Security Error')
...
describe('BashTool — Stderr Scenarios', () => {
  ...
  it('should handle stderr-only output', async () => {
    ...
    const result = await tool.execute({ command: 'echo "error only" >&2' }, makeContext())
    expect(result.isError).toBeFalsy()
  ...
  it('should handle both empty stdout and empty stderr gracefully', async () => {
    ...
    const result = await tool.execute({ command: 'cd /tmp' }, makeContext())
