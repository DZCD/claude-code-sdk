 ...
 * when the buffer is full. Maintains insert order for retrieval.
 */
export class CircularBuffer<T> {
  ...
  /**
   * Get the most recent N items from the buffer, ordered from oldest to newest.
   */
  getRecent(count: number): T[] {
    ...
    for (let i = 0; i < available; i++) {
      const index = (start + this.size - available + i) % (this.capacity || 1)
  ...
  toArray(): T[] {
    ...
    for (let i = 0; i < this.size; i++) {
      const index = (start + i) % (this.capacity || 1)
      result.push(this.buffer[index]!)
    }
